...
import AccessoriesPage, { Product as AccessoriesProduct } from "./AccessoriesPage";
...
export async function generateMetadata({ searchParams }: { searchParams?: { category?: string; vehicleModel?: string } }): Promise<Metadata> {
  ...
  let apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/products?accessories=true`;
  ...
  const products: AccessoriesProduct[] = (Array.isArray(data.data) ? data.data : []).map((p: any) => ({
    _id: p._id,
    name: p.name,
    slug: p.slug,
    price: p.price,
...
const langCookie = cookieStore.get('language');
  ...
  const title = seoProduct?.metaTitle?.[currentLang] || seoProduct?.name?.[currentLang] || 'Accessories Products';
  const description = seoProduct?.metaDescription?.[currentLang] || seoProduct?.description?.[currentLang] || 'Explore Accessories engines, transmissions, swaps and more.';
  const image = seoProduct?.imageUrl || '/assets/hero.png';
  const pageUrl = `${process.env.NEXT_PUBLIC_API_URL}/accessories${categorySlug ? `?category=${categorySlug}` : ''}`;

  // Structured data (JSON-LD)
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Product",
    "name": seoProduct?.name?.[currentLang] || "Accessories Product",
    "image": [image],
...
export default async function Page({
  ...
  products.forEach((p) => {
    ...
    const match = name.match(/accessories\s+(\w+)/i);
