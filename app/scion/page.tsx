...
export default function ScionPage() {
  ...
  params.set("scion", "true"); // keep this always
  ...
  router.push(`/scion?${params.toString()}`, { scroll: false });
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const scion = await translate("scion");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?scion=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?scion=true";
...
router.push(`/scion?category=${slug}`);
...
>
        ...
        <ol className="inline-flex items-center space-x-2">
          ...
          <li>/</li>
          <li className="text-gray-900 font-medium">{translatedTexts.scion}</li>
