...
export default function InfinitiPage() {
  ...
  params.set("infiniti", "true"); // keep this always
  ...
  router.push(`/infiniti?${params.toString()}`, { scroll: false });
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const infiniti = await translate("Infiniti");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?infiniti=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?infiniti=true";
...
router.push(`/infiniti?category=${slug}`);
...
>
        ...
        <ol className="inline-flex items-center space-x-2">
          ...
          <li>/</li>
          <li className="text-gray-900 font-medium">{translatedTexts.infiniti}</li>
