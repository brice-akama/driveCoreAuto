...
export default function TransmissionsScionManualPage() {
  ...
  router.push(`/transmissions/scion/manual?${params.toString()}`);
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  const [translatedTexts, setTranslatedTexts] = useState({
    ...
    addToWishList: "Add to Wishlist",
      home: " Home",
      toyota: "Transmissions Scion Manual",
  ...
  async function fetchTranslations() {
    try {
      ...
      const toyota = await translate("Transmissions Scion Manual");
      ...
      setTranslatedTexts({
        ...
        home: home || "Home",
        toyota: toyota || "Transmissions Scion Manual",
    ...
    (async () => {
      try {
        ...
        productList.forEach((p) => {
          ...
          const match = name.match(/Transmissions Scion Manual\s+(\w+)/i);
          if (match && match[1]) {
...
useEffect(() => {
  ...
  <BrandLinksNav currentBrand="Transmissions Scion Manual" />
</div>
