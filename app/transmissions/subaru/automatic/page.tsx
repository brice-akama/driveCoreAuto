...
export default function TransmissionsSubaruAutomaticPage() {
    ...
    const updateUrl = (vehicleModel?: string | null, engineCode?: string | null) => {
        ...
        params.set("transmissionsSaburaAutomatic", "true");  // keep this always
        ...
        router.push(`/transmissions/subaru/automatic?${params.toString()}`);  // transmissionsHondataAutomatic
    ...
    const [translatedTexts, setTranslatedTexts] = useState({
        ...
        home: " Home",
        subaruAutomatic: "Subaru Transmissions Automatic",
    ...
    useEffect(() => {
        async function fetchTranslations() {
            try {
                ...
                const subaruAutomatic = await translate("Subaru Transmissions Automatic");
                ...
                setTranslatedTexts({
                    ...
                    home: home || "Home",
                    subaruAutomatic: subaruAutomatic || "Subaru Transmissions Automatic",
    ...
    useEffect(() => {
        (async () => {     //
            ...
            try {
                const res = await fetch("/api/products?transmissionsSaburaAutomatic=true");
                ...
                productList.forEach((p) => {
                    ...
                    const match = name.match(/Saburu\s+(\w+)/i);
                    if (match && match[1]) {
    ...
    useEffect(() => {
        (async () => {
            setLoading(true);
            let apiUrl = "/api/products?transmissionsSaburaAutomatic=true";
    ...
    const extractModel = (name: string): string => {
        ...
        return match ? match[0].toUpperCase() : "SUBARU ENGINE";
    };
    ...
    const selectEngineCode = (code: string) => {
        ...
        router.push(`/transmissions/subaru/automatic?category=${slug}`);
    ...
    return (
        ...
        <>
            <div className=" md:mt-0">
                ...
                <BrandLinksNav currentBrand="Subaru Transmissions Automatic" />
            </div>
