...
export default function TransmissionsSubaruManualPage() {
  ...
  router.push(`/transmissions/subaru/manual?${params.toString()}`);
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  const [translatedTexts, setTranslatedTexts] = useState({
    ...
    addToWishList: "Add to Wishlist",
      home: " Home",
      toyota: "Transmissions Subaru Manual",
  ...
  async function fetchTranslations() {
    try {
      ...
      const toyota = await translate("Transmissions Subaru Manual");
      ...
      setTranslatedTexts({
        ...
        home: home || "Home",
        toyota: toyota || "Transmissions Subaru Manual",
    ...
    (async () => {
      try {
        ...
        productList.forEach((p) => {
          ...
          const match = name.match(/Transmissions Subaru Manual\s+(\w+)/i);
          if (match && match[1]) {
...
useEffect(() => {
  ...
  <BrandLinksNav currentBrand="Transmissions Subaru Manual" />
</div>
