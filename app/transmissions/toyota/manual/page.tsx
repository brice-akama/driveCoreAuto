...
import TransmissionsToyotaManualPage, { Product as TransmissionsToyotaManualProduct } from './TransmissionsToyotaManualPage';
...
export async function generateMetadata({ searchParams }: { searchParams?: { category?: string; vehicleModel?: string } }): Promise<Metadata> {
  ...
  let apiUrl = `${process.env.NEXT_PUBLIC_API_URL}/api/products?transmissionsToyotaManuel=true`;
  ...
  const products: TransmissionsToyotaManualProduct[] = (Array.isArray(data.data) ? data.data : []).map((p: any) => ({
    _id: p._id,
    name: p.name,
    slug: p.slug,
    price: p.price,
...
const langCookie = cookieStore.get('language');
  ...
  const title = seoProduct?.metaTitle?.[currentLang] || seoProduct?.name?.[currentLang] || 'Transmissions Toyota Manual Products';
  const description = seoProduct?.metaDescription?.[currentLang] || seoProduct?.description?.[currentLang] || 'Explore Transmissions Toyota Manual engines, transmissions, swaps and more.';
  const image = seoProduct?.imageUrl || '/assets/hero.png';
  const pageUrl = `${process.env.NEXT_PUBLIC_API_URL}/transmissionsToyotaManuel${categorySlug ? `?category=${categorySlug}` : ''}`;

  // Structured data (JSON-LD)
  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "Product",
    "name": seoProduct?.name?.[currentLang] || "transmissions Toyota Manual Product",
    "image": [image],
