...
export default function TransmissionsAcuraManualPage() {
  ...
  router.push(`/transmissions/acura/manual?${params.toString()}`);
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  const [translatedTexts, setTranslatedTexts] = useState({
    ...
    addToWishList: "Add to Wishlist",
      home: " Home",
      toyota: "Transmissions Acura Manual",
  ...
  async function fetchTranslations() {
    try {
      ...
      const toyota = await translate("Transmissions Acura Manual");
      ...
      setTranslatedTexts({
        ...
        home: home || "Home",
        toyota: toyota || "Transmissions Acura Manual",
    ...
    (async () => {
      try {
        ...
        productList.forEach((p) => {
          ...
          const match = name.match(/Transmissions Acura Manual\s+(\w+)/i);
          if (match && match[1]) {
...
useEffect(() => {
  ...
  <BrandLinksNav currentBrand="Transmissions Acura Manual" />
</div>
