...
interface ForgotPasswordPageProps {
  t: Record<string, string>;               // SSR translations
  initialLanguage?: string;                // SSR language
}
...
const ForgotPasswordPage: React.FC<ForgotPasswordPageProps> = ({
  ...
  const { language, translations: contextT, setLanguage } = useLanguage();

  // Set initial SSR language and translations
  useEffect(() => {
    if (initialLanguage) setLanguage(initialLanguage, 'forgot-Password');
  }, [initialLanguage, setLanguage]);

  // Use translations from SSR initially, or fallback to context
  const t = contextT || initialT || {};
  ...
  return (
    <div className="mt-20 lg:mt-40">
      <div className="bg-black text-white py-8 text-center w-full">
        <h1 className="text-4xl font-black">{t['forgotPasswordTitle']}</h1>
        <Breadcrumb />
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
        <div className="max-w-md w-full bg-white rounded-lg shadow-md p-8 space-y-6">
          ...
          <p className="text-sm text-center text-gray-500">
            {t['rememberPassword']}{' '}
            <Link href={`/${language}/profile`} className="text-blue-600 underline hover:text-blue-700">
              {t['login']}
            </Link>
          </p>
        </div>
