...
export default function SubaruSwapsPage() {
  ...
  params.set("swapsSubaru", "true"); // keep this always
  ...
  router.push(`/swaps/subaru?${params.toString()}`);   // 
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const toyota = await translate("Swaps Subaru");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?swapsSubaru=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?swapsSubaru=true";
...
router.push(`/swaps/subaru?category=${slug}`);
...
useEffect(() => {
  ...
  <BrandLinksNav currentBrand="Swaps Subaru" />
</div>
