...
export default function SwapsHondaPage() {
  ...
  params.set("swapsHonda", "true"); // keep this always
  ...
  router.push(`/swaps/honda?${params.toString()}`);
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const toyota = await translate("Swaps Honda");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?swapsHonda=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?swapsHonda=true";
...
router.push(`/swaps/honda?category=${slug}`);
setShowMobileFilters(false);
...
useEffect(() => {
  ...
  <BrandLinksNav currentBrand="Swaps Honda" />
</div>
