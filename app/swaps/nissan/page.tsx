...
export default function SwapsNissanPage() {
  ...
  params.set("swapsNissan", "true"); // keep this always
  ...
  router.push(`/swaps/nissan?${params.toString()}`);
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const toyota = await translate("Swaps Nissan");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?swapsNissan=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?swapsNissan=true";
...
router.push(`/swaps/nissan?category=${slug}`);
setShowMobileFilters(false);
...
useEffect(() => {
  ...
  <BrandLinksNav currentBrand="Swaps Nissan" />
</div>
