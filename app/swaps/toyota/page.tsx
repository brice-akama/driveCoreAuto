...
// File: driveCoreAuto/app/swaps/toyota/page.tsx
...
export default function SwapsToyotaPage() {
  ...
  params.set("swapsToyota", "true"); // keep this always
  ...
  router.push(`/swaps/toyota?${params.toString()}`);
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const toyota = await translate("Swaps Toyota");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?swapsToyota=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?swapsToyota=true";
...
router.push(`/swaps/toyota?category=${slug}`);
setShowMobileFilters(false);
...
useEffect(() => {
  ...
  <BrandLinksNav currentBrand="Swaps Toyota" />
</div>
