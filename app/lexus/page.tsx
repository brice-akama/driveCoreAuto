...
export default function LexusPage() {
  ...
  params.set("lexus", "true"); // keep this always
  ...
  router.push(`/lexus?${params.toString()}`, { scroll: false });
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const lexus = await translate("Lexus");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?lexus=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?lexus=true";
...
router.push(`/lexus?category=${slug}`);
...
useEffect(() => {
  ...
  <BrandLinksNav currentBrand="lexus" />
</div>
...
>
        ...
        <ol className="inline-flex items-center space-x-2">
          ...
          <li>/</li>
          <li className="text-gray-900 font-medium">{translatedTexts.lexus}</li>
