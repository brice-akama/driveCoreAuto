...
export default function HondaPage() {
  ...
  params.set("honda", "true"); // keep this always
  ...
  router.push(`/honda?${params.toString()}`, { scroll: false });
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const honda = await translate("honda");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?honda=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?honda=true";
...
router.push(`/honda?category=${slug}`);
...
>
        ...
        <ol className="inline-flex items-center space-x-2">
          ...
          <li>/</li>
          <li className="text-gray-900 font-medium">{translatedTexts.honda}</li>
