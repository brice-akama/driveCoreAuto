...
const [hoveredIcon, setHoveredIcon] = useState<string | null>(null);
  ...
  params.set("subaru", "true"); // keep this always
  ...
  router.push(`/subaru?${params.toString()}`, { scroll: false });
};
 ...
 const productsPerPage = isMobile ? 6 : 12;
  ...
  async function fetchTranslations() {
    try {
      ...
      const subaru = await translate("Subaru");
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/products?subaru=true");
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      let apiUrl = "/api/products?subaru=true";
...
router.push(`/subaru?category=${slug}`);
...
>
        ...
        <ol className="inline-flex items-center space-x-2">
          ...
          <li>/</li>
          <li className="text-gray-900 font-medium">{translatedTexts.subaru}</li>
